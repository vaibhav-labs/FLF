#!/usr/bin/env python3
"""
epistack_eval.py  -  combined epistack evaluation (v5, four cases: three starter shapes + a real large base)
...
VERSION = "combined, v6 (4 cases; covid_large adds Part C retracted-claim test)"
...
def main():
    ...
    report = {"case": a.case, "shape": cfg["shape"], "question": cfg["top_q"], "model": a.model,
              ...
              "caveat": "LLM personas, few runs. Suggestive, not proof. " + kb_note}
    if cfg["double_count"]:
    ...
    if cfg.get("retraction"):
        report["PART_C_retracted_claim"] = score_retraction(
