...
The linkage test measures all three constraints it applies: reasoning from the base only, updating on a held-out claim it never saw, and holding up under a counter-argument. The first is enforced by instruction; the next two are measured directly. It also reports the objective metric, calibration gained per claim read. On the large base it also runs a double-counting probe and a retracted-claim provenance probe (Part C).
